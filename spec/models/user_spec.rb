...
RSpec.describe User, type: :model do
  describe 'バリデーション' do
    ...
    it '目標距離がない場合は無効であること' do
      user = FactoryBot.build(:user, goal_meters: nil)
      user.valid?
      expect(user.errors[:goal_meters]).to include('を入力してください')
    end

    it '目標距離が100,000より大きい場合は無効であること' do
      user = FactoryBot.build(:user, goal_meters: 100_001)
      user.valid?
      expect(user.errors[:goal_meters]).to include('は100000以下の値にしてください')
    end
  end
  ...
  describe '#display_avatar_url' do
    ...
    context 'avatar_type: uploaded' do
      before do
        ...
        user.uploaded_avatar.attach(io: File.open(Rails.root.join('spec/fixtures/files/avatar.jpg')),
