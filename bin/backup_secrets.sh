...
# ~/workspace/tekumemo/bin/backup_secrets.sh をコマンドで実行
# chmod +x ~/workspace/tekumemo/bin/backup_secrets.sh 実行権限を付与
...
# ==========================================
# 4.5. データベースダンプ (.sqlファイル)
# ==========================================
if [ -d "backups" ]; then
    echo "💾 backups/*.sql を検索中..."

    # 復元時に分かりやすいよう backups ディレクトリを作って配置
    mkdir -p "$TEMP_DIR/backups"

    # backups ディレクトリ内から .sql ファイルを検索し、コピー
    find backups -type f -name "*.sql" | while read sql_file; do
        cp "$sql_file" "$TEMP_DIR/backups/" && echo "✅ $sql_file (DBダンプ) を追加"
        FILES_FOUND=true
    done

    # .sqlファイルが見つからなかった場合でも、FILES_FOUNDは変更しない
