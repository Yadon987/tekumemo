...
export default class extends Controller {
  ...
  showOverlay() {
    if (!this.overlay) {
      ...
      this.overlay.className = "fixed inset-0 bg-black/70 z-[9998] transition-opacity duration-300 opacity-0"
  ...
  highlightElement(element) {
    ...
    if (!this.spotlight) {
      ...
      this.spotlight.className = "fixed pointer-events-none z-[9999] transition-all duration-300 rounded-full border-4 border-white/50 shadow-[0_0_30px_rgba(255,255,255,0.5)] animate-pulse"
  ...
  showTooltip(target, step) {
    ...
    this.tooltip.className = "fixed z-[10000] w-64 max-w-[90vw] transition-all duration-300 opacity-0 transform translate-y-4"
    ...
    this.tooltip.innerHTML = `
      <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-xl p-4 border border-gray-100 dark:border-slate-700 relative">
        ...
        <h3 class="font-bold text-lg text-gray-800 dark:text-white mb-2">${step.title}</h3>
        <p class="text-sm text-gray-600 dark:text-slate-300 mb-4 whitespace-pre-wrap leading-relaxed">${step.message}</p>

        <div class="flex justify-between items-center">
          <div class="flex space-x-1">
            ${this.steps.map((_, i) => `
              <div class="w-1.5 h-1.5 rounded-full ${i === this.currentStepIndex ? 'bg-blue-500' : 'bg-gray-200 dark:bg-slate-600'}"></div>
            `).join('')}
          </div>
    ...
    // イベントリスナー
    document.getElementById("coach-skip-btn").addEventListener("click", () => this.complete())
    ...
    // 画面中央に寄せるためのオフセット
    const left = Math.max(10, Math.min(window.innerWidth - finalRect.width - 10, rect.left + rect.width / 2 - finalRect.width / 2))
